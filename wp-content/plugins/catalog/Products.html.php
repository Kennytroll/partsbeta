	...
	<option value="0"';
        ...
        foreach ($cat_row as $catt) {
            ...
            if ((isset($_POST['cat_search']) && $_POST['cat_search'] == $catt->id) || (isset($_GET['categoryid']) && $_GET["categoryid"] == $catt->id))
              $serch_fields .= 'selected="selected"';
            $serch_fields .= '>' . esc_js(esc_html(stripslashes($catt->name))) . '</option>';
        ...
        <input type="hidden" name="asc_or_desc" id="asc_or_desc"
               value="<?php if (isset($_POST['asc_or_desc'])) echo esc_js(esc_html(stripslashes($_POST['asc_or_desc']))); ?>"/>
        <input type="hidden" name="order_by" id="order_by"
               value="<?php if (isset($_POST['order_by'])) echo esc_js(esc_html(stripslashes($_POST['order_by']))); ?>"/>
    ...
    <tr>
        ...
        <td>
            <select name="ordering">
                ...
                ?>
                <option
                    value="<?php if($ord_elem[$i - 1]->ordering) echo $ord_elem[$i - 1]->ordering + 1; else echo 0; ?>"><?php if($ord_elem[$i - 1]->ordering) echo $ord_elem[$i - 1]->ordering + 1; else echo 0; ?>
                    Last
                </option>
            </select>
