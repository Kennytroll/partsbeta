...
function  wdcat_showProducts()
{
    ...
    if (isset($_POST['search_events_by_title']))
        $_POST['search_events_by_title'] = esc_js(esc_html(stripslashes($_POST['search_events_by_title'])));
    if (isset($_POST['asc_or_desc']))
        $_POST['asc_or_desc'] = esc_js(esc_html(stripslashes($_POST['asc_or_desc'])));
    if (isset($_POST['order_by']))
        $_POST['order_by'] = esc_js(esc_html(stripslashes($_POST['order_by'])));
    ...
    if (isset($_POST['page_number'])) {
        ...
        if ($_POST['asc_or_desc']) {
            ...
            if ($_POST['asc_or_desc'] == 1) {
                ...
                $sort["1_or_2"] = "2";
                $order = "ORDER BY " . esc_js(esc_html(stripslashes($sort["sortid_by"]))) . " ASC";
            } else {
                $sort["custom_style"] = "manage-column column-title sorted desc";
                $sort["1_or_2"] = "1";
                $order = "ORDER BY " . esc_js(esc_html(stripslashes($sort["sortid_by"]))) . " DESC";
            }
        }
    ...
    if (isset($_POST['search_events_by_title'])) {
        $search_tag = esc_js(esc_html(stripslashes($_POST['search_events_by_title'])));
    } else {
        $search_tag = "";
    }
    if (isset($_GET["categoryid"])) {
        ...
        if (isset($_POST['cat_search'])) {
            $categ_id = esc_js(esc_html(stripslashes($_POST['cat_search'])));
        } else {
            $categ_id = 0;
        }
...
//var_dump($_POST['cat_search']);die();
    $savedd = $wpdb->update($wpdb->prefix . 'spidercatalog_products', array(
            'name' => esc_js(esc_html(stripslashes($_POST['name']))),
            'category_id' => esc_js(esc_html(stripslashes($_POST['categ_search']))),
            'description' => $script_cat,
            'image_url' => esc_js(esc_html(stripslashes($new_images))),
            'cost' => esc_js(esc_html(stripslashes($_POST['cost']))),
            'market_cost' => esc_js(esc_html(stripslashes($_POST['market_cost']))),
            'param' => esc_js(esc_html(stripslashes($_POST['param']))),
            'ordering' => esc_js(esc_html(stripslashes($_POST['ordering']))),
            'published' => esc_js(esc_html(stripslashes($_POST['published']))),
            'published_in_parent' => esc_js(esc_html(stripslashes($_POST['par_cat']))),
        ),
        array('id' => $id),
        array(
            '%s',
...
function   spider_cat_prod_rev($id)
{
    ...
    $where = " WHERE product_id='" . esc_js(esc_html(stripslashes($id))) . "'";
    ...
    $query = "SELECT * FROM " . $wpdb->prefix . "spidercatalog_product_reviews" . $where . " " . $order . " " . " LIMIT " . esc_js(esc_html(stripslashes($limit))) . ",20";
    $rows = $wpdb->get_results($query);
...
function wdcat_delete_rev($id)
{
    ...
    global $wpdb;
    $cid = array();
    $cid = esc_js(esc_html(stripslashes($_POST['post'])));
...
function   spider_cat_prod_rating($id)
{
    ...
    if (isset($_POST['page_number'])) {
        ...
        if ($_POST['asc_or_desc']) {
            $sort["sortid_by"] = esc_js(esc_html(stripslashes($_POST['order_by'])));
    ...
    $query = "SELECT * FROM " . $wpdb->prefix . "spidercatalog_product_votes" . $where . " " . $order . " " . " LIMIT " . esc_js(esc_html(stripslashes($limit))) . ",20";
    $rows = $wpdb->get_results($query);
