...
$_POST['page_number']=esc_js(esc_html(stripslashes($_POST['page_number'])));
  $where='';
	...
	if(isset($_POST['search_events_by_title'])){
	 ...
	 if(isset($_GET["catid"])){
       ...
       if(isset($_POST['cat_search'])){
		$cat_id=esc_js(esc_html(stripslashes($_POST['cat_search'])));
		}else{
...
function wdcat_save_cat()
{
	 ...
	 if(isset($_POST["ordering"])){	 
	 	$rows=$wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'spidercatalog_product_categories WHERE ordering>='.esc_js(esc_html(stripslashes($_POST["ordering"]))).'   AND parent='.esc_js(esc_html(stripslashes($_POST['parent']))).' ORDER BY `ordering` ASC ');
	 }
	 else{
		 		echo "<h1>Error</h1>";
		return false;
    ...
    for($i=0;$i<$kk;$i++){
	 ...
	 $save_or_no= $wpdb->insert($wpdb->prefix.'spidercatalog_product_categories', array(
		...
		'parent'                 => esc_js(esc_html(stripslashes($_POST["parent"]))),
        'category_image_url'     => esc_js(esc_html(stripslashes($new_images))),
