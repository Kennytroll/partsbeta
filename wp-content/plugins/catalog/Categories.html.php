...
This section allows you to create categories of products. <a href="http://web-dorado.com/spider-catalog-wordpress-guide-step-2.html" target="_blank" style="color:blue; text-decoration:none;">More...</a></td> 
	...
	foreach($cat_row as $cat_id)
	{
      ...
      if((isset($_POST['cat_search']) && $_POST['cat_search'] == $cat_id->id) || (isset($_GET["catid"]) && $_GET["catid"]==$cat_id->id)) {
		...
		$serch_fields.='>'.esc_js(esc_html(stripslashes($cat_id->name))).'</option>';
...
?>
<option value="<?php if($ord_elem[$i-1]->ordering+1) echo $ord_elem[$i-1]->ordering+1; else echo 0; ?>" ><?php if($ord_elem[$i-1]->ordering+1) echo  $ord_elem[$i-1]->ordering+1; else echo 0;  ?> Last</option>
</select>
...
?>
<option value="<?php if(isset($ord_elem[$i-1])) echo  $ord_elem[$i-1]->ordering+1; else echo 0; ?>"><?php if(isset($ord_elem[$i-1])) echo  $ord_elem[$i-1]->ordering+1; echo 0; ?> Last</option>
</select>
