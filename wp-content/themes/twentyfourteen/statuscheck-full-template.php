...
<!-- hide div containing table of main content of status check and show div with ul postlist clone/appended/showed
 -->
<br /><br />List source:  <form method="post">
  <input type="radio" name="list" value="list1" checked> Kinetrol Notes Spreadsheet (Log 06, as of Mar 2016)<br>
  <input type="radio" name="list" value="list2"> Active WP Posts<br>
  <input type="radio" name="list" value="list3"> Enter Custom CSV list <input type="textarea" name="custom" value="" style="display:none;"><span id="custominfo" style="display:none"> Example: 077-100,057-020,107-......</span><br>

  <input type="submit" name="Submit">
</form> 

<!-- get post values and assign to a variable/  do if check echo  -->

<?php 

$listorino = 1;  // set default list source


if ($_POST["list"] == "list1" ) {  
	$listorino = 1;
	echo "<Br />first list defaultish";
}elseif ($_POST["list"] == "list2") {$listorino = 2;}
elseif ($_POST["list"] == "list3") {$listorino = 3; if(isset($_POST['custom']) && $_POST['custom'] != "") { $customlist = $_POST['custom'];} else {echo "Empty post list; enter comma separated list of posts to check"; }}




// if list1 then use this, then elseif for case 2
...
if($listorino == 2) {$listarray = $array1;};
if($listorino == 3 && isset($customlist)) {$listarray = explode(",", $customlist);};
?>

<hr><div id="bundhandler">
 <div id="statustable">
 	<div id="togglecols"> Toggle Column:<div id="togglecolslist" style="padding-left:13px;">
 		<input type="checkbox" name="CA File" class="togcheck" datacol="1" checked> CA File |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="2" checked> CA Contents |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="3" checked> WP Posted |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="4" checked> Active Rev |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="5" checked> PDF |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="6" checked> STEP |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="7" checked> SLDDRW |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="8" checked> SLDPRT |
 		<input type="checkbox" name="CA File" class="togcheck" datacol="9" checked> DWG 
...
jQuery('input[value=list3]').click(function() {jQuery('input[name="custom"]').show(); jQuery('#custominfo').show();});

/*jQuery('#bundaroo_filter').append("wuttttttttttttttttttt");*/
jQuery('#bundaroo_filter').after(jQuery('#togglecols'));
